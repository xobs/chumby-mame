...
{
	Uint32 val=0;
	fprintf(stderr, "Processing button %s: %d\n", pressed?"up":"down", button);
	switch(button)
	{
		case 6://11
			val=GP2X_START; break;
	...
	//	case 12://12
			...
			//break;
		default:
			fprintf(stderr, "Unrecognized button: %d\n", button);
			return;
	}
	if (pressed)
		(*st) |= val;
...
static void joyhatprocess(Uint32 *st, Uint8 value)
{
	if(value & SDL_HAT_UP)
		(*st) |= GP2X_UP;
	else
		...
		switch(event.type)
		{
			...
			case SDL_JOYAXISMOTION:
				if (event.jaxis.axis==0)
				{
					...
					fprintf(stderr, "Axis 0 movement! Value: %d\n", event.jaxis.value);
...
//					{
				...
				else if (event.jaxis.axis==1)
				{
					...
					fprintf(stderr, "Axis 1 movement! Value: %d\n", event.jaxis.value);
					if (event.jaxis.value<-18000)
					{
						if (!reset_yu){
...
//					{
			...
			case SDL_JOYHATMOTION:
				joyhatprocess((Uint32 *)&_st_, event.jhat.value);
